...
import { ArrowLeft, BookOpen, Award, ShieldCheck, ChevronRight } from "lucide-react";
        ...
        <section className="mx-auto max-w-4xl px-4 sm:px-6 py-6 md:py-10 w-full flex flex-col gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start w-full">
                ...
                {trackModules.length > 0 ? (
                  <div className="flex flex-col gap-4">
                    {trackModules.map((m) => (
                      <div
                        key={m.slug}
                        className="p-4 rounded-xl border border-card-border bg-[#030712]/60 flex flex-col gap-3 group"
                      >
                        ...
                        <div className="flex justify-between items-center text-[10px] font-mono border-t border-card-border/40 pt-2.5">
                          <span className="text-text-muted">Submodules: {m.submodules.length}</span>
                          <Link
                            href={`/ai-engineer/tracks/${track.slug}/${m.slug}`}
            ...
            <div className="md:col-span-4 flex flex-col gap-6 w-full">
              ...
              <div className="bg-[#030712] border border-card-border/60 rounded-xl p-5 flex flex-col gap-3">
                <h4 className="text-[10px] font-bold text-foreground uppercase tracking-widest border-b border-card-border/40 pb-2">
                  Planned Projects
                </h4>
                <div className="flex flex-col gap-2">
                  {trackProjects.map((p) => {
                    ...
                    return (
                      <div key={p.slug} className="flex justify-between items-center text-[10px] py-1 border-b border-card-border/20 last:border-0">
                        <span className="text-text-muted truncate max-w-[140px]">{p.title}</span>
